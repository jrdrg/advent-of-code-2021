let exampleInputStr = "be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
let inputStr = "ceb bgfdea febgc ec eadcgfb eagbcd fcdebg dcef gafbc egdbf | fdbgec fedbg gdabefc gefbd
...
let parseLine = (line: string) => {
  let split = line |> Js.String.split(" | ")

  let signalStr = split[0]
  let outputStr = split[1]

  let patterns = signalStr |> String.split_on_char(' ') |> Array.of_list
  let outputValue = outputStr |> String.split_on_char(' ') |> Array.of_list

  {patterns: patterns, outputValue: outputValue}
}

let makeSet = (segments: array<string>) => {
  Belt.Set.String.fromArray(segments)
}

let digitMap =
  Belt.Map.Int.empty
  ->Belt.Map.Int.set(0, makeSet(["a", "b", "c", "e", "f", "g"]))
  ->Belt.Map.Int.set(1, makeSet(["c", "f"]))
  ->Belt.Map.Int.set(2, makeSet(["a", "c", "d", "e", "g"]))
  ->Belt.Map.Int.set(3, makeSet(["a", "c", "d", "f", "g"]))
  ->Belt.Map.Int.set(4, makeSet(["b", "c", "d", "f"]))
  ->Belt.Map.Int.set(5, makeSet(["a", "b", "d", "f", "g"]))
  ->Belt.Map.Int.set(6, makeSet(["a", "b", "d", "e", "f", "g"]))
  ->Belt.Map.Int.set(7, makeSet(["a", "c", "f"]))
  ->Belt.Map.Int.set(8, makeSet(["a", "b", "c", "d", "e", "f", "g"]))
  ->Belt.Map.Int.set(9, makeSet(["a", "b", "c", "d", "f", "g"]))

let signalCountMap = Belt.Map.Int.reduce(digitMap, Belt.Map.Int.empty, (map, digit, signalSet) => {
  let count = Belt.Set.String.size(signalSet)
  Belt.Map.Int.update(map, count, v =>
    switch v {
    | Some(counts) => Some(Belt.Set.Int.add(counts, digit))
    | None => Some(Belt.Set.Int.add(Belt.Set.Int.empty, digit))
    }
  )
})

let decodeEasy = (pattern: string) => {
  let signals = pattern |> Js.String.split("")
  let size = Array.length(signals)

  let possible = signalCountMap->Belt.Map.Int.get(size)
  switch possible {
  | Some(counts) =>
    if Belt.Set.Int.size(counts) == 1 {
      let vals = Belt.Set.Int.toArray(counts)
      Some(vals[0])
    } else {
      None
    }
...
let getEasyDigitCount = (entry: entry) => {
  entry.outputValue |> Array.fold_left((sum, digitSignals) => {
    switch decodeEasy(digitSignals) {
    | Some(_) => sum + 1
    | None => sum
    }
  }, 0)
...
let part1 = () => {
  ...
  input |> Array.fold_left((sum, entry) => {
    sum + getEasyDigitCount(entry)
  }, 0)
}
...
exception InvalidDigit(int)

type letter = Found(int) | Possible(array<int>)

let findLetter = (pattern: string) => {
  let signals = pattern |> Js.String.split("")
  let size = Array.length(signals)
  ...
  | None => raise(InvalidLength)
  }
}

let optionPatternToSet = (pattern: option<string>) => {
  switch pattern {
  | Some(p) => Some(Belt.Set.String.fromArray(p |> Js.String.split("")))
  | None => None
  }
}

let canBeDigits = (pattern: string, decoded: Belt.Map.String.t<int>) => {
  let setSize = Belt.Set.String.size

  let letters = pattern |> Js.String.split("") |> Belt.Set.String.fromArray
  let patternByDigit = Belt.Map.String.reduce(decoded, Belt.Map.Int.empty, (
    map,
    pattern,
    digit,
  ) => {
    Belt.Map.Int.set(map, digit, pattern)
  })
  let digit1 = Belt.Map.Int.get(patternByDigit, 1) |> optionPatternToSet
  let digit4 = Belt.Map.Int.get(patternByDigit, 4) |> optionPatternToSet
  let digit7 = Belt.Map.Int.get(patternByDigit, 7) |> optionPatternToSet
  let digit8 = Belt.Map.Int.get(patternByDigit, 8) |> optionPatternToSet

  switch Belt.Set.String.size(letters) {
  | 5 =>
    // 2, 3, 5
    switch (digit1, digit4, digit7, digit8) {
    | (_, Some(d4), Some(d7), Some(d8)) =>
      let intersect7 = Belt.Set.String.intersect(letters, d7)
      let intersect8 = Belt.Set.String.intersect(letters, d8)
      let intersect4 = Belt.Set.String.intersect(letters, d4)

      // if all the segments of 7 are in the string, it's 3
      if setSize(intersect7) == setSize(d7) {
        Found(3)
      } else if (
        // if all the segments are in 8, and 3 of them are in 4, then it's 5
        setSize(intersect8) == 5 && setSize(intersect4) == 3
      ) {
        Found(5)
      } else {
        Found(2)
  ...
  | 6 =>
    ...
    | (Some(d1), Some(d4), _, _) =>
      let intersect4 = Belt.Set.String.intersect(letters, d4)
      let intersect1 = Belt.Set.String.intersect(letters, d1)

      if setSize(intersect4) == setSize(d4) {
        Found(9)
      } else if setSize(intersect1) == setSize(d1) {
        Found(0)
      } else {
        Found(6)
      }
  ...
  | _ =>
    switch Belt.Map.String.get(decoded, pattern) {
    | Some(p) => Found(p)
    | None => Possible([])
    }
  }
...
let rec decodeLine = (
  ...
  decoded: Belt.Map.String.t<int>,
  letterMapping: Belt.Map.String.t<string>,
) => {
  switch patterns {
  | list{head, ...rest} =>
    switch findLetter(head) {
    | Found(digit) =>
      let updated = Belt.Map.String.set(decoded, head, digit)
      decodeLine(rest, updated, letterMapping)

    | Possible(_) =>
      let possible = canBeDigits(head, decoded)
      switch possible {
      | Found(found) =>
        let updated = Belt.Map.String.set(decoded, head, found)
        decodeLine(rest, updated, letterMapping)
      | _ =>
        let newList = List.append(rest, list{head})
        decodeLine(newList, decoded, letterMapping)
      }
    }
  | _ => decoded
  }
...
let decodeOutput = (patterns: array<string>, decoded: Belt.Map.String.t<int>) => {
  let digits =
    patterns
    |> Array.map(p => {
      let letters = p |> Js.String.split("") |> Belt.Set.String.fromArray

      Belt.Map.String.findFirstBy(decoded, (pattern, _) => {
        let patternSet = pattern |> Js.String.split("") |> Belt.Set.String.fromArray
        let intersection = Belt.Set.String.intersect(letters, patternSet)
        let isEqual =
          Belt.Set.String.size(intersection) == Belt.Set.String.size(patternSet) &&
            Belt.Set.String.size(intersection) >= Belt.Set.String.size(letters)
...
let part2 = () => {
  ...
  let outputDigits = input |> Array.map(entry => {
    let decoded = decodeLine(
      Array.to_list(entry.patterns),
      Belt.Map.String.empty,
      Belt.Map.String.empty,
    )

    decodeOutput(entry.outputValue, decoded)
  })

  outputDigits |> Array.fold_left((sum, digit) => {
    sum + digit
  }, 0)
}
