...
function findFuelCost(crabs, hPos) {
  return List.fold_left((function (sum, cost) {
                return sum + cost | 0;
              }), 0, List.map((function (c) {
                    return Pervasives.abs(c - hPos | 0);
                  }), crabs));
}

function findLowestFuelCost(crabs, _start, end, _lowest) {
  while(true) {
    var lowest = _lowest;
    var start = _start;
    if (start > end) {
    ...
    var cost = findFuelCost(crabs, start);
    var newLow = lowest !== undefined && cost >= lowest ? lowest : cost;
    _lowest = newLow;
    _start = start + 1 | 0;
    continue ;
  };
...
function part1(param) {
  ...
  var match = minMax(input);
  return findLowestFuelCost(input, match[0], match[1], undefined);
}

console.log("Part 1", part1(undefined));

function fuelCostForStep(_step, _cost) {
  while(true) {
    var cost = _cost;
    var step = _step;
    if (step === 0) {
...
function findAdjustedFuelCost(crabs, hPos) {
  return List.fold_left((function (sum, cost) {
                return sum + fuelCostForStep(cost, 0) | 0;
              }), 0, List.map((function (c) {
                    return Pervasives.abs(c - hPos | 0);
                  }), crabs));
}

function findLowestAdjustedFuelCost(crabs, _start, end, _lowest) {
  while(true) {
    var lowest = _lowest;
    var start = _start;
    if (start > end) {
    ...
    var cost = findAdjustedFuelCost(crabs, start);
    var newLow = lowest !== undefined && cost >= lowest ? lowest : cost;
    _lowest = newLow;
    _start = start + 1 | 0;
    continue ;
...
function part2(param) {
  var input = parseInput(inputStr);
  var match = minMax(input);
  return findLowestAdjustedFuelCost(input, match[0], match[1], undefined);
}

console.log("Part 2", part2(undefined));

var exampleInputStr = "16,1,2,0,4,2,7,1,2,14";

exports.exampleInputStr = exampleInputStr;
exports.inputStr = inputStr;
exports.parseInput = parseInput;
exports.minMax = minMax;
exports.findFuelCost = findFuelCost;
exports.findLowestFuelCost = findLowestFuelCost;
exports.part1 = part1;
exports.fuelCostForStep = fuelCostForStep;
exports.findAdjustedFuelCost = findAdjustedFuelCost;
exports.findLowestAdjustedFuelCost = findLowestAdjustedFuelCost;
exports.part2 = part2;
/*  Not a pure module */
